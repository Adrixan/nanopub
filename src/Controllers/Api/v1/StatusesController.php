...
final class StatusesController
{
     ...
     */
    public function favouritedBy(Request $request, int $id): void
    {
        $status = Status::find($id);
        ...
        try {
            $rows = Database::fetchAll(
                'SELECT a.* FROM likes l JOIN accounts a ON l.account_id = a.id WHERE l.status_id = ? ORDER BY l.created_at DESC LIMIT ?',
                [$id, $limit]
            );
        ...
        $accounts = array_map(fn(array $row): array => $this->formatAccount($row), $rows);
     ...
     */
    public function rebloggedBy(Request $request, int $id): void
    {
        $status = Status::find($id);
        ...
        try {
            $rows = Database::fetchAll(
                'SELECT a.* FROM statuses s JOIN accounts a ON s.account_id = a.id WHERE s.reblog_of_id = ? ORDER BY s.created_at DESC LIMIT ?',
                [$id, $limit]
            );
        } catch (\RuntimeException $e) {
            $rows = [];
