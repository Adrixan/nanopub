...
final class SearchService
{
     ...
     * @param int $limit Maximum results
     * @return array<Status> Array of statuses
     */
    public function searchStatuses(string $query, int $accountId, int $limit = 40): array
    {
        $query = trim($query);
        ...
        $params = array_merge([$searchTerm], $followingIds, [$accountId, $limit]);
     ...
     */
    public function searchHashtags(string $query, int $limit = 40): array
    {
        $query = trim($query);
     ...
     */
    public function search(string $query, int $accountId, string $type = 'all', int $limit = 40): array
    {
        $results = [
            'accounts' => [],
            'statuses' => [],
        ...
        if ($type === 'all' || $type === 'accounts') {
            $results['accounts'] = $this->searchAccounts($query, $limit);
        }
        
        if ($type === 'all' || $type === 'statuses') {
            $results['statuses'] = $this->searchStatuses($query, $accountId, $limit);
        }
        
        if ($type === 'all' || $type === 'hashtags') {
            $results['hashtags'] = $this->searchHashtags($query, $limit);
        }
